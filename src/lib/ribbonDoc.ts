 ...
 *
 * Кнопки «Панель движка» больше нет: она возвращала родную ленту Univer «со
 * всем, чего пока нет в нашей», то есть держала два места для одного действия
 * на неопределённый срок. Чего не хватает — доносится сюда, а не обходится
 * чужой панелью.
 *
