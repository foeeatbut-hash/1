...
type State =
  | { name: 'idle' }
  | { name: 'stale' }
  | { name: 'ready'; kind: 'text' | 'table' | 'image'; lines: number; chars: number; cells: number; truncated: number }
    ...
    <div className="w-full h-full p-1.5 select-none" style={{ background: 'transparent' }}>
      <div className="w-full h-full rounded-2xl overflow-hidden bg-white dark:bg-slate-900
        ...
        <div className="flex items-center gap-2 px-3 py-2">
          ...
          {basket > 0 && (
            <span className="ml-auto text-2xs font-bold text-slate-500 dark:text-slate-400">
              в корзине {basket}
            </span>
          )}
        </div>
