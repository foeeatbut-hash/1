 ...
 *
 * Разделителей до двух, а не один, намеренно: иначе «стр. 4» вообще не
 * находится и отброшенное некуда показать. Пусть найдётся и честно ляжет
 * в счётчик «Отброшено», а не пропадёт молча.
 */
const LOOSE = /[A-Za-zА-Яа-яЁё]{1,8}[-_. ]{0,2}\d{1,5}[A-Za-zА-Яа-яЁё]{0,3}(?:[-.]\d{1,3})?/g;
